...
public object RotaryScrollableDefaults {
    ...
    public fun behavior(
        ...
        val viewConfiguration = RotaryViewConfiguration()
     ...
     *
     * Upstream asks the package manager for `android.hardware.rotaryencoder.lowres`. Off-Android
     * the host is the one that knows what it is portraying, so it says so through the device
     * configuration; a mouse wheel is high-res, which is the default.
     */
    @Composable
    private fun isLowResInput(): Boolean =
        LocalWearDeviceConfiguration.current.isLowResRotaryInput
...
private fun flingBehavior(
    ...
    isLowRes: Boolean,
    viewConfiguration: RotaryViewConfiguration,
): RotaryScrollableBehavior {
...
internal class RotaryFlingHandler(
    ...
    init {
        // Upstream picks the per-input-device limits on API 34+ and the global ones below it.
        // RotaryViewConfiguration answers both the same way, so the version check is gone rather
        // than resolved one way: there is no older platform here to be compatible with.
        minFlingSpeed =
            viewConfiguration
            ...
            flow.collectLatest { event ->
                ...
                with(behavior) {
                    performScroll(
                        ...
                        // Compose Multiplatform's RotaryScrollEvent carries no input device id —
                        // it is an Android MotionEvent concept. Nothing downstream distinguishes
                        // devices any more (see RotaryViewConfiguration), so 0 stands for "the
                        // one rotary input this host has".
                        inputDeviceId = 0,
                        orientation = orientation,
                    )
                }
...
// `MotionEvent.AXIS_SCROLL` and `InputDevice.SOURCE_ROTARY_ENCODER`. They are only ever passed to
// RotaryViewConfiguration, which ignores them, but they are kept rather than deleted so the call
// sites stay identical to upstream's and the patch stays small.
private const val AxisScroll = 26

private const val RotaryInputSource = 0x00400000
