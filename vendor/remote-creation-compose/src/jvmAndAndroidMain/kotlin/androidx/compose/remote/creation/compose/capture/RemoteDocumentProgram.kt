 ...
 *
 * Unlike [DocumentOp.SaveRestore] trees which form the main hierarchical tree, [DocumentTransform]s are flat,
 ...
 * [DocumentOp.Transform] nodes for final flushing.
 */
internal sealed class DocumentTransform {
    ...
    class Translate(val dx: RemoteFloat, val dy: RemoteFloat) : DocumentTransform() {
    ...
    class Rotate(val angle: RemoteFloat, val px: RemoteFloat?, val py: RemoteFloat?) : DocumentTransform() {
        override val isIdentity: Boolean
    ...
    class Skew(val sx: RemoteFloat, val sy: RemoteFloat) : DocumentTransform() {
        override val isIdentity: Boolean
...
internal sealed class DocumentOp {
     ...
     * operation.
     */
    open fun optimizeChildScopes(program: RemoteDocumentProgram) {}
     ...
     *
     * @param program The [RemoteDocumentProgram] running the optimization.
     * @return True if this operation should be pruned, false otherwise.
     */
    open fun shouldPrune(program: RemoteDocumentProgram): Boolean = false
     ...
     *
     * @param dest The destination list accumulating flattened [DocumentOp] nodes.
     */
    open fun flattenInto(dest: MutableList<DocumentOp>) {
        dest.add(this)
    }
     ...
     * [RemoteDocumentProgram.SpanOp] wrappers, chaining their execution order and rewiring
     * dependent operations across the scope boundary.
     *
     * @param spanOp The wrapping [RemoteDocumentProgram.SpanOp] representing this operation in the
     *   span graph.
     * @param span The containing [RemoteDocumentProgram.Span] whose operations are being flattened.
     * @param dest The destination list accumulating flattened [RemoteDocumentProgram.SpanOp] nodes.
     */
    open fun flattenInto(
        spanOp: RemoteDocumentProgram.SpanOp,
        span: RemoteDocumentProgram.Span,
        dest: MutableList<RemoteDocumentProgram.SpanOp>,
    ) {
        dest.add(spanOp)
    }
    ...
    class Draw private constructor(
        ...
        constructor(
            operation: WriterOp,
            beforeWrite: (() -> Unit)?,
        ) : this(false, operation, beforeWrite, null)
    ...
    class Transform(val op: DocumentTransform) : DocumentOp() {
        ...
        // Emits a matrix transformation command to the document writer.
        override fun emitsWireCommands(): Boolean = true
    ...
    class SaveRestore(
        ...
        val children: MutableList<DocumentOp> = ArrayList(),
    ) : DocumentOp() {
        /**
        ...
        var spanOp: RemoteDocumentProgram.SpanOp? = null
        ...
        override fun optimizeChildScopes(buffer: RemoteDocumentProgram) {
        ...
        private fun flattenChildren() {
            val newChildren = ArrayList<DocumentOp>(children.size)
            for (i in 0 until children.size) {
        ...
        override fun flattenInto(dest: MutableList<DocumentOp>) {
            flattenChildren()
            when (elisionMode) {
                ElisionMode.DISCARD -> {}
        ...
        override fun flattenInto(
            ...
            when (elisionMode) {
                ...
                ElisionMode.INLINE -> {
                    var lastInlinedOp: RemoteDocumentProgram.SpanOp? = null
                    for (j in 0 until children.size) {
                        val newSpanOp = RemoteDocumentProgram.SpanOp(span, children[j])
                        if (j == 0) {
     ...
     *
     * Typically [childSpan] will be a [RemoteDocumentProgram.Span], which if hasChildCommands will
     * return false if empty.
     *
     * @property condition The condition that controls execution of the child span.
     * @property childSpan The child [RemoteDocumentProgram.Span] holding conditional operations.
    ...
    class DrawConditionally(
        ...
        override fun shouldPrune(buffer: RemoteDocumentProgram) = !childSpan.emitsWireCommands()

        override fun toString(): String = "DrawConditionally(${condition.toDebugString()})"
    }

    /** Redirects a child span to an offscreen bitmap and then restores the previous target. */
    class DrawToBitmap(
        val bitmap: RemoteImageBitmap,
        val previousBitmapId: Int,
        val clearColor: Int?,
        val childSpan: RemoteDocumentProgram.Span,
        val forcePaintRefresh: () -> Unit,
    ) : DocumentOp() {
        override fun write(writer: RemoteComposeWriter, creationState: RemoteComposeCreationState) {
            ...
            childSpan.record(writer, creationState)
            forcePaintRefresh()
            writer.drawOnBitmap(previousBitmapId, 1, 0)
        }
        ...
        override fun optimizeChildScopes(program: RemoteDocumentProgram) {
            program.optimizeSpan(childSpan)
        }

        override fun toString(): String = "DrawToBitmap(clearColor=$clearColor)"
    }

    /** Repeats a child span over a remote float range. */
    class Loop(
        val indexId: Int,
        val from: RemoteFloat,
        val step: RemoteFloat,
        ...
        override fun shouldPrune(program: RemoteDocumentProgram): Boolean =
            !childSpan.emitsWireCommands()

        override fun toString(): String = "Loop(indexId=$indexId)"
    }
    ...
    class CustomComponent(
        ...
        override fun shouldPrune(buffer: RemoteDocumentProgram): Boolean = false
 ...
 * the document.
 */
internal inline fun <T> List<T>.evaluateElisionPass(isTopLevel: Boolean, op: (T) -> DocumentOp) {
    var pendingSave: DocumentOp.SaveRestore? = null
    for (i in 0 until size) {
        val current = op(this[i])
        if (current is DocumentOp.SaveRestore) {
            current.isTopLevel = isTopLevel
            current.evaluateElision()
            if (current.elisionMode == DocumentOp.ElisionMode.DISCARD) continue
            pendingSave?.let {
                if (it.hasChildTransformsOrClips()) it.elisionMode = DocumentOp.ElisionMode.PRESERVE
            }
            pendingSave = current
...
/**
 * Typed, scoped Remote Compose program retained until all document-level optimization passes have
 * completed. Recording adapters are producers of this IR; they are not part of its serialization
 * contract.
 */
internal class RemoteDocumentProgram(val enableOptimizations: Boolean = false) {
    ...
    internal class Span(val parent: Span?, val depth: Int) {
         ...
         * [DocumentOp.DrawConditionally]) and added directly to this list for linear wire emission.
         */
     ...
     * spans.
     */
    internal class SpanOp(var idealSpan: Span, val op: DocumentOp) {
        val deps = ArrayList<SpanOp>()
        var visited = false
    }

    private var declarationRoot = Span(null, 0)
    private var lastDeclarationOp: SpanOp? = null
     ...
     */
    public fun recordRenderingOp(op: DocumentOp): SpanOp {
    ...
    /** Records a declaration in the preamble, independently of the current body scope. */
    internal fun recordDeclaration(action: () -> Unit): SpanOp {
        val spanOp = SpanOp(declarationRoot, DocumentOp.Declaration(action))
        declarationRoot.operations.add(spanOp)
        lastDeclarationOp?.let { spanOp.deps.add(it) }
        lastDeclarationOp = spanOp
        return spanOp
    }
    ...
    /** Returns a string representation of the document program's current span tree. */
    override fun toString(): String {
        val hasDeclarations =
            declarationRoot.operations.isNotEmpty() || declarationRoot.child != null
        val hasBody = spanTreeRoot.operations.isNotEmpty() || spanTreeRoot.child != null
        if (!hasDeclarations && !hasBody) return "RemoteDocumentProgram(empty)"
        return buildString {
            append("RemoteDocumentProgram(")
            if (hasDeclarations) append("declarations=").append(declarationRoot)
            if (hasDeclarations && hasBody) append(", ")
            if (hasBody) append("body=").append(spanTreeRoot)
            append(")")
        }
    }
     ...
     * 4. Resets the program state, clearing maps and resetting the span tree root for the next use.
     *
    ...
    internal fun optimize(creationState: RemoteComposeCreationState) {
        commonSubExpressionElimination(creationState)

        if (enableOptimizations) {
            optimizeSpan(declarationRoot)
            declarationRoot.sortAllSpans()
    ...
    internal fun writeTo(
        ...
        spanTreeRoot.record(writer, creationState)

        declarationRoot = Span(null, 0)
        lastDeclarationOp = null
     ...
     * - Past [DocumentTransform.Scale]: Offsets are scaled by the scale factors.
     * - Past [DocumentTransform.Rotate]: Offsets are rotated by the rotation angle.
     * - Past [DocumentTransform.Skew]: Offsets are skewed by the skew factors.
     *
     ...
     */
    private fun MutableList<DocumentTransform>.pushTranslate(dx: RemoteFloat, dy: RemoteFloat) {
        var currDx = dx
        var currDy = dy
        var i = size - 1
        ...
        while (i >= 0) {
            val op = this[i]
            when (op) {
                is DocumentTransform.Translate -> {
                    // Found another Translate, fuse them by adding offsets.
                    this[i] = DocumentTransform.Translate(op.dx + currDx, op.dy + currDy)
                    return
                }
                is DocumentTransform.Scale -> {
        ...
        this.add(i + 1, DocumentTransform.Translate(currDx, currDy))
    }

    /**
     * Pushes a scale operation into the pending operations list.
     *
     * If the immediate preceding operation is also a [DocumentTransform.Scale] and shares the exact same
     ...
     */
    private fun MutableList<DocumentTransform>.pushScale(
        sx: RemoteFloat,
        sy: RemoteFloat,
        px: RemoteFloat?,
        py: RemoteFloat?,
        ...
        if (isNotEmpty()) {
            ...
            if (
                last is DocumentTransform.Scale && last.px.cacheKeysMatch(px) && last.py.cacheKeysMatch(py)
            ) {
                // Fuse by multiplying the scale factors.
                this[size - 1] = DocumentTransform.Scale(last.sx * sx, last.sy * sy, px, py)
                return
            }
        }
        // Otherwise, append the new scale.
        add(DocumentTransform.Scale(sx, sy, px, py))
    }

    /**
     * Pushes a rotation operation into the pending operations list.
     *
     * If the immediate preceding operation is also a [DocumentTransform.Rotate] and shares the exact same
     ...
     */
    private fun MutableList<DocumentTransform>.pushRotate(
        angle: RemoteFloat,
        px: RemoteFloat?,
        py: RemoteFloat?,
    ) {
        if (isNotEmpty()) {
            ...
            if (
                last is DocumentTransform.Rotate && last.px.cacheKeysMatch(px) && last.py.cacheKeysMatch(py)
            ) {
                // Fuse by adding the rotation angles.
                this[size - 1] = DocumentTransform.Rotate(last.angle + angle, px, py)
                return
            }
        }
        // Otherwise, append the new rotation.
        add(DocumentTransform.Rotate(angle, px, py))
    }
     ...
     */
    private fun MutableList<DocumentTransform>.pushSkew(sx: RemoteFloat, sy: RemoteFloat) {
        if (isNotEmpty()) {
            val last = last()
            if (last is DocumentTransform.Skew) {
                if (last.sy.constantValueOrNull == 0f && sy.constantValueOrNull == 0f) {
                    this[size - 1] = DocumentTransform.Skew(last.sx + sx, sy)
                    return
                }
                if (last.sx.constantValueOrNull == 0f && sx.constantValueOrNull == 0f) {
                    this[size - 1] = DocumentTransform.Skew(sx, last.sy + sy)
                    return
                }
            }
        }
        add(DocumentTransform.Skew(sx, sy))
    }

    /**
     * Fuses and commutes a list of consecutive [DocumentTransform] transforms.
     *
     ...
     */
    private fun optimizeTransformList(ops: List<DocumentTransform>): List<DocumentTransform> =
        buildList(ops.size) {
            for (i in 0 until ops.size) {
                val op = ops[i]
                when (op) {
                    is DocumentTransform.Translate -> pushTranslate(op.dx, op.dy)
                    is DocumentTransform.Scale -> pushScale(op.sx, op.sy, op.px, op.py)
                    is DocumentTransform.Rotate -> pushRotate(op.angle, op.px, op.py)
                    is DocumentTransform.Skew -> pushSkew(op.sx, op.sy)
                }
            }
     ...
     *
     * Within each [DocumentOp.SaveRestore] node, consecutive [DocumentOp.Transform] nodes (separated
     ...
     */
    internal fun optimizeTransforms(ops: MutableList<DocumentOp>) {
        for (i in 0 until ops.size) {
            val child = ops[i]
            if (child is DocumentOp.SaveRestore) {
                optimizeTransforms(child.children)
            }
        }

        val pendingTransforms = ArrayList<DocumentTransform>()
        val newOps =
            buildList(ops.size) {
                fun flushTransforms() {
                    if (pendingTransforms.isNotEmpty()) {
                        ...
                        for (j in 0 until optimized.size) {
                            add(DocumentOp.Transform(optimized[j]))
                        }
     ...
     *
     * Fuses consecutive [DocumentOp.Transform] nodes in the span's operations list, ensuring that any
        ...
        val newOps =
            buildList(span.operations.size) {
                fun flushTransforms() {
                    ...
                    val pendingTransforms = ArrayList<DocumentTransform>(pendingSpanOps.size)
                    for (k in 0 until pendingSpanOps.size) {
                        pendingTransforms.add((pendingSpanOps[k].op as DocumentOp.Transform).op)
                    }
                    val optimized = optimizeTransformList(pendingTransforms)
                    ...
                    for (j in 0 until optimized.size) {
                        val newSpanOp = SpanOp(span, DocumentOp.Transform(optimized[j]))
                        if (j == 0) {
     ...
     * [DocumentOp.evaluateElision].
     *
     * A block is marked as [DocumentOp.ElisionMode.INLINE] if it contains drawing calls but no state
     * modifications (transforms or clips). Blocks with no drawing calls at all are marked as
     * [DocumentOp.ElisionMode.DISCARD]. All other blocks preserve their boundaries by default.
     */
    ...
    /**
     * Flattens (inlines) elided [DocumentOp.SaveRestore] nodes that reside directly in a [Span] via
     * [DocumentOp.flattenInto].
     */
    ...
    private fun emitExpression(
        ...
        if (state is BaseRemoteState<*>) {
            ...
            val op = SpanOp(idealSpan, DocumentOp.Expression(key, state))
