...
public abstract class RemoteFloat internal constructor(cacheKey: RemoteStateCacheKey) :
     ...
     */
    public fun toRemoteString(format: DecimalFormat = DefaultJavaDecimalFormat): RemoteString =
        formatRemoteFloatJvm(this, format)
    ...
    public companion object {
        ...
        public fun createNamedRemoteFloat(
            ...
            return RemoteFloatExpression(
                ...
                floatArrayOf(
                    creationState.writer.addNamedFloat(domain.prefixed(name), defaultValue)
                )
            }
        }
            ...
            return RemoteFloatExpression(
                ...
                if (Utils.isVariable(initialValueId)) {
                    ...
                    creationState.writer.setNamedVariable(
                        ...
                        NamedVariableType.FLOAT,
                    )
                    floatArrayOf(floatId)
                } else {
                    val floatId =
                        creationState.writer.addNamedFloat(domain.prefixed(name), initialValueId)
                    floatArrayOf(floatId)
                }
            }
        }
...
internal constructor(
    ...
    internal constructor() :
        this(
            ...
            idProvider = { creationState -> creationState.writer.reserveFloatVariable() },
        )
...
internal constructor(
    ...
    public constructor(
        ...
        cacheKey = RemoteStateInstanceKey(),
        arrayProvider = { creationState ->
            val floatArrayId = creationState.writer.addFloatArray(value())
...
internal constructor(
    ...
    public fun getAnimationTime(): Float {
        return start.elapsedNow().inWholeNanoseconds * 1E-9f
    }
}
...
internal fun combineToFloatArray(
    ...
    if (totalSizeInline > MAX_SAFE_FLOAT_ARRAY) {
        ...
        for (array in arrays) {
            array.copyInto(combinedArray, idx)
            idx += array.size
        }
    }
...
internal val DefaultJavaDecimalFormat: DecimalFormat = DecimalFormat()

internal fun DecimalFormat.toJavaTextFromFloatOptions(): TextFromFloatOptions {
...
private fun formatRemoteFloatJvm(value: RemoteFloat, format: DecimalFormat): RemoteString {
    val (before, after, flags) = format.toJavaTextFromFloatOptions()

    if (format.minimumFractionDigits == 0 && after > 0 && format !== DefaultJavaDecimalFormat) {
